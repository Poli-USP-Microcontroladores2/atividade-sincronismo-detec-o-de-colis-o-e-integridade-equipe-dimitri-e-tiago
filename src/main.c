...
void main(void)
{
    ...
    while (1) {
        ...
        // ----------- Lê da UART1 (outra placa) e envia para UART0 -----------
        if (uart_poll_in(uart1, &c) == 0) {
            uart_poll_out(uart0, c); // retransmite o caractere
            
            // Se recebeu '\n' ou '\r', garante a quebra de linha
            if (c == '\n' || c == '\r') {
                // Adiciona quebra de linha extra se necessário
